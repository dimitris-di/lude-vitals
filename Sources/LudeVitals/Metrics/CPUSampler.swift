...
final class CPUSampler: AnySampler {
    ...
    private var lastPopoverIsOpen = false
    ...
    func sample(context: SamplingContext) async -> CPUMetrics {
        if lastPopoverIsOpen && !context.popoverIsOpen {
            previousPidTicks.removeAll(keepingCapacity: false)
            previousProcSample = 0
        }
        lastPopoverIsOpen = context.popoverIsOpen
        ...
        guard numCpu > 0, Int(infoCount) >= Int(numCpu) * states else { return .zero }
        ...
        var totUser: Double = 0, totSys: Double = 0, totIdle: Double = 0, totNice: Double = 0, totAll: Double = 0
        ...
        for (i, c) in current.enumerated() {
            ...
            totUser += dUser; totSys += dSys; totIdle += dIdle; totNice += dNice; totAll += dAll
            switch type {
        ...
        let total = totAll > 0 ? (totUser + totSys + totNice) / totAll : 0
        ...
        return CPUMetrics(
            ...
            loadAverage: LoadAverage(one: load[0], five: load[1], fifteen: load[2]),
            topProcesses: context.popoverIsOpen ? topProcesses() : []
        )
    }
    ...
    private func topProcesses() -> [ProcessUsage] {
        ...
        let cap = (Int(bytesNeeded) / pidStride) * 2 + 64
        var pids = [pid_t](repeating: 0, count: cap)
        let returnedBytes = pids.withUnsafeMutableBufferPointer { buf -> Int32 in
            proc_listpids(UInt32(PROC_ALL_PIDS), 0, buf.baseAddress, Int32(cap * pidStride))
        }
        if returnedBytes <= 0 { return [] }
        let realCount = min(Int(returnedBytes) / pidStride, pids.count)
        ...
        for idx in 0..<realCount {
            ...
            let cpu: Double
            if let prev = prev, elapsed > 0, totalTicks >= prev {
            ...
            let rawName = n > 0 ? String(cString: nameBuf) : "pid \(pid)"
            let name = Fmt.sanitizeDisplayString(rawName, maxLength: 64)
            let row = ProcessUsage(id: pid, name: name, cpu: cpu, memoryBytes: info.pti_resident_size)
            insertTop(row, into: &topCPU, limit: topLimit) {
                if $0.cpu != $1.cpu { return $0.cpu > $1.cpu }
                if $0.memoryBytes != $1.memoryBytes { return $0.memoryBytes > $1.memoryBytes }
                return $0.id < $1.id
            }
            insertTop(row, into: &topMemory, limit: topLimit) {
                if $0.memoryBytes != $1.memoryBytes { return $0.memoryBytes > $1.memoryBytes }
                if $0.cpu != $1.cpu { return $0.cpu > $1.cpu }
                return $0.id < $1.id
            }
        ...
        var unionByPID: [Int32: ProcessUsage] = [:]
        unionByPID.reserveCapacity(topLimit * 2)
        for row in topCPU { unionByPID[row.id] = row }
        for row in topMemory { unionByPID[row.id] = row }
        return unionByPID.values.sorted {
            if $0.cpu != $1.cpu { return $0.cpu > $1.cpu }
            if $0.memoryBytes != $1.memoryBytes { return $0.memoryBytes > $1.memoryBytes }
            return $0.id < $1.id
        }
    ...
    private func insertTop(
        _ row: ProcessUsage,
        into rows: inout [ProcessUsage],
        limit: Int,
        isHigherPriority: (ProcessUsage, ProcessUsage) -> Bool
    ) {
        rows.append(row)
        var index = rows.count - 1
        while index > 0 && isHigherPriority(rows[index], rows[index - 1]) {
